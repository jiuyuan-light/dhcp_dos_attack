...
DHCPC_CTRL::DHCPC_CTRL(QWidget* parent) :QWidget(parent), datalist(new QStringList)
{
    ...
    netboxLabel = new QLabel(QString::fromLocal8Bit("网卡:"));
    CtrlLayout = new QGridLayout(this);
    BtnLayout = new QHBoxLayout();

    client_cnt = new QLabel(QString::fromLocal8Bit("DHCP客户端数量:"));
...
void DHCPC_CTRL::GetNetworkCardName(QList<QString>& card)
{
    ...
    for (int i = 0; i < list.count(); i++)
    {
        ...
        //        detail=detail+QString::fromLocal8Bit("设备：")+interface.humanReadableName()+"\n";
    }
...
{
    DetailBtn = new QPushButton(QString::fromLocal8Bit("详细配置"));
...
void DHCPC_CTRL::StartOperator()
{
    if (netbox->currentIndex() == -1) {
        QMessageBox::information(this, QString::fromLocal8Bit("提示"), QString::fromLocal8Bit("必须选择一个网卡"));
        return;
    }

    int nums = get_dhcpc_nums();
    if (nums > USER_NUMS) {
        QMessageBox::information(this, QString::fromLocal8Bit("提示"), QString::fromLocal8Bit("不允许超过") + QString::number(USER_NUMS));
        return;
    }
