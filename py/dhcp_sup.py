...
class PseudoThread():
    @staticmethod
    def async_run_now(*args):
        for func in args:
            PseudoThread.async_run_later_event(func)
    @staticmethod
    def async_run_later_event(func, delay = 0, *args, **kwargs):
        gevent.sleep(delay)
...
class sup_manage(object):
    ...
    def get_mac_by_arp(self, iface, src_ip, dst_ip, src_mac):
        ...
        responses, unanswered = srp(arp_req, timeout=1, iface = iface)
